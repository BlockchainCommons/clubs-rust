...
use crate::{
    Error, Result,
    frost::{FrostGroup, FrostSignatureShares, FrostSigningPackage},
};
...
impl FrostGroup {
    ...
    pub fn attach_preaggregated_signature(
        ...
        signed
            .verify_signature_from(&self.verifying_signing_key())
            .map_err(|e| {
                Error::msg(format!(
                    "envelope signature verification failed: {e}"
                ))
            })?;
    ...
    pub fn aggregate_and_attach_signature(
        ...
        let frost_pkg = self.to_frost_public_key_package()?;
        ...
        for share in &shares_g.shares {
            ...
            let id = self.id_for_xid(&share.xid)?;
            let share =
                frost_secp256k1_tr::round2::SignatureShare::deserialize(
                    &share.share,
                )
                .map_err(|e| {
                    Error::msg(format!("deserialize signature share: {e}"))
                })?;
        ...
        self.attach_preaggregated_signature(envelope, &sig_bytes)
    }
}
