...
impl From<FrostPmGammaShare> for Envelope {
    fn from(value: FrostPmGammaShare) -> Self {
        let mut e = Envelope::new(known_values::UNIT);
        e = e.add_type("FrostPmGammaShare");
...
impl TryFrom<Envelope> for FrostPmGammaShare {
    type Error = Error;

    fn try_from(envelope: Envelope) -> Result<Self> {
        envelope.check_type_envelope("FrostPmGammaShare")?;
        let subj_env = envelope.subject();
        let kv = subj_env.try_known_value()?;
        if kv.value() != known_values::UNIT.value() {
            return Err(Error::msg("unexpected subject for FrostPmGammaShare"));
        }
...
mod tests {
    ...
    #[test]
    fn frost_pm_gamma_share_roundtrip_text() {
        ...
        let share = FrostPmGammaShare { xid, session, gamma_bytes: [0x33; 33] };
        let env: Envelope = share.clone().into();
        #[rustfmt::skip]
        let expected = (indoc! {r#"
            '' [
                'isA': "FrostPmGammaShare"
                "gamma": Bytes(33)
        ...
        let rt = FrostPmGammaShare::try_from(env).unwrap();
        assert_eq!(share, rt);
    }
}
